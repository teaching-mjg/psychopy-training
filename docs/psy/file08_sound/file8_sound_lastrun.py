...
expName = 'file8_sound'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\mgreen\\gits\\psychopy-training-1-of-2\\psy\\file08_sound\\file8_sound_lastrun.py',
...
prompt = visual.TextStim(win=win, name='prompt',
    text='How many syllables in the word above?\nType the number then press Enter.',
    font='Open Sans',
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if ISI.status == NOT_STARTED and t >= 0-frameTolerance:
            ...
            sound_1.setSound(soundid, secs=-1)
    ...
    t0 = timer.getTime()
    sound_1.setSound(soundid, hamming=True)
    sound_1.setVolume(1.0, log=False)
    stimulus.setText(word)
    textbox.reset()
    # keep track of which components have finished
    trialComponents = [sound_1, stimulus, textbox, prompt]
    ...
    while continueRoutine:
        ...
        if elapsed_time >= 5:
            ...
            thisExp.addData("response_acc", response_acc)
            continueRoutine=False
        
        typed = textbox.getText()
        if typed:
            if typed[len(typed)-1]=="\n":
                response = typed[:-1]
                response_time = elapsed_time
                response_acc = str(response) == str(nsyllables)
                thisExp.addData("response", response)
                thisExp.addData("response_time", response_time)
                thisExp.addData("response_acc", response_acc)
                continueRoutine=False
