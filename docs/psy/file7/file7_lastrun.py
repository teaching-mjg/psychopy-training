...
expName = 'file7'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\mgreen\\gits\\psychopy-training-1-of-2\\psy\\file7\\file7_lastrun.py',
...
# --- Initialize components for Routine "evaluate_response" ---
...
# --- Initialize components for Routine "feedback_averages" ---
# Run 'Begin Experiment' code from code_2
mean_rt_message=""
mean_ac_message=""
rtmess = visual.TextStim(win=win, name='rtmess',
    text='',
    font='Open Sans',
    ...
    languageStyle='LTR',
    depth=-1.0);
acmess = visual.TextStim(win=win, name='acmess',
    text='',
    font='Open Sans',
...
for thisTrial in trials:
    ...
    # --- Prepare to start Routine "evaluate_response" ---
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # keep track of which components have finished
    evaluate_responseComponents = []
    for thisComponent in evaluate_responseComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "evaluate_response" ---
    for thisComponent in evaluate_responseComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "evaluate_response" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "feedback" ---
    continueRoutine = True
    # update component parameters for each repeat
    text.setColor(feedback_colour, colorSpace='rgb')
    ...
    # --- Prepare to start Routine "feedback_averages" ---
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from code_2
    mean_rt_message="Your mean RT is: "+str(round(mean_rt,3)*1000)+"ms"
    mean_ac_message="Your mean accuracy is: " +str(round(mean_ac,2)*100)+"%"
    rtmess.setText(mean_rt_message)
    acmess.setText(mean_ac_message)
    # keep track of which components have finished
    feedback_averagesComponents = [rtmess, acmess]
    for thisComponent in feedback_averagesComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        # *rtmess* updates
        if rtmess.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            rtmess.frameNStart = frameN  # exact frame index
            rtmess.tStart = t  # local t and not account for scr refresh
            rtmess.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(rtmess, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'rtmess.started')
            rtmess.setAutoDraw(True)
        if rtmess.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > rtmess.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
                rtmess.tStop = t  # not accounting for scr refresh
                rtmess.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rtmess.stopped')
                rtmess.setAutoDraw(False)
        
        # *acmess* updates
        if acmess.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            acmess.frameNStart = frameN  # exact frame index
            acmess.tStart = t  # local t and not account for scr refresh
            acmess.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(acmess, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'acmess.started')
            acmess.setAutoDraw(True)
        if acmess.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > acmess.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
                acmess.tStop = t  # not accounting for scr refresh
                acmess.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'acmess.stopped')
                acmess.setAutoDraw(False)
    ...
    # --- Ending Routine "feedback_averages" ---
    for thisComponent in feedback_averagesComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # using non-slip timing so subtract the expected duration of this Routine
